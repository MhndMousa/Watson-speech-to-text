...
//  Watson-Lynn
//
//  Created by Muhannad Alnemer on 9/20/19.
...
class WordCloudViewController: UIViewController {
    ...
    @IBAction func burronClicked(_ sender: Any) {
        let headers = [
            "x-rapidapi-host": "wordcloudservice.p.rapidapi.com",
            "x-rapidapi-key": "d96e0d06b3mshcbbd261d2f3af6ep1e432fjsn3d6832842d54",
        ...
        do{
            ...
            let dataTask = session.dataTask(with: request as URLRequest, completionHandler: { (data, response, error) -> Void in
                ...
                do {
                    ...
                    let gitData = try JSONDecoder().decode(DataSet.self, from: data)
                    print("response data:", gitData.url)
                    self.image.sd_setImage(with: URL(string: gitData.url!))
//                    NSAssertionHandler(gitData)
                } catch let err {
                    print("Err", err)
                }
